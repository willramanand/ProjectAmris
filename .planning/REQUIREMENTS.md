...
- [ ] **API-01**: Cross-component consistency audit — dimension matrices per event, prop, boolean-naming, default, slot, part, and `--am-*` token across all ~67 components
...
- [ ] **API-04**: Slot names, `::part()` names, and `--am-*` tokens enumerated and treated as frozen public surface
- [ ] **API-05**: Committed CEM baseline `api/custom-elements.baseline.json` with report-only surface diff during cleanup
