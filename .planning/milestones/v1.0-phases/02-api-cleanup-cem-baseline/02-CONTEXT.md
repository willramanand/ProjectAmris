...
- New components or new registered custom elements — feature freeze at 67 (clarification added during Phase 2 planning revision: the 66 component files register **79 unique custom-element tagNames**, including 13 compound/sub-elements — e.g. am-accordion-item, am-tab, am-option, am-menu-item; "67" was an approximate top-level count. The authoritative freeze scope is the full registered-element SET of 79 tagNames, verified as a set comparison via `npm run diff:surface`). Extracted machinery is non-registered and non-exported.
...
- **D-06 [informational]:** No MIGRATION.md this phase — rely on the aggregated changelog Changesets generate. A polished old→new migration guide is Phase 5 (Documentation). — **Reversibility:** reversible. *(Scope-fence decision NOT to produce an artifact this phase; no plan implements it. The MIGRATION.md prohibition is carried in the API-02 rename plans' Deferred/Prohibited sections; tracked out-of-phase against Phase 5 per line 117.)*
