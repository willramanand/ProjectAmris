...
- [x] **Phase 9: Runtime-Performance Tuning** - Cut re-render/reposition churn on the heaviest components against the post-deferral baseline, with a11y DOM provably intact (completed 2026-08-23)
- [x] **Phase 10: Graceful Degradation & Compatibility Matrix** - Below-floor browsers degrade instead of silently failing; widened WebKit/Firefox/Chromium lane validates it against a documented true floor (completed 2026-08-27)
- [x] **Phase 11: Gate Enforcement & Cost Publication** - Flip size + runtime-count budgets to enforcing (wall-clock stays report-only) and publish per-component cost cards (completed 2026-08-31)
...
- [x] 09-01-PLAN.md — TRACER: countMethod harness probe + data-grid identity-keyed sort memo + sortComputes drop + data-grid a11y snapshot (RPERF-01, RPERF-04, wave 1)
...
- [x] 09-02-PLAN.md — combobox two-level filter memo (no debounce) + filterCalls drop + combobox a11y snapshot (RPERF-02, RPERF-04, wave 2)
- [x] 09-03-PLAN.md — shared-controller reposition-churn cache (costly, browser-lane gate) + CR-01 lazy-load retry fold-in + middlewareBuilds + popover a11y snapshot (RPERF-03, RPERF-04, D-05, wave 2)
...
- [x] 10-01-PLAN.md — capabilities.ts + attachInternalsSafe() tracer on am-input (COMPAT-01, COMPAT-02, wave 1)
- [x] 10-02-PLAN.md — form-participation.ts + compat-forms opt-in subpath + Changeset, reversibility-gated (COMPAT-03, wave 1)
- [x] 10-03-PLAN.md — CSS @supports selector(:has(*)) guard audit, 6 files/10 rules (COMPAT-06, wave 1)
- [x] 10-04-PLAN.md — combined COMPAT-02/03 rollout batch A1, 6 files, own test files (COMPAT-02, COMPAT-03, wave 2)
- [x] 10-05-PLAN.md — combined COMPAT-02/03 rollout batch A2, 5 files, own test files (COMPAT-02, COMPAT-03, wave 2)
- [x] 10-06-PLAN.md — combined COMPAT-02/03 rollout batch B, 4 files, own test files (COMPAT-02, COMPAT-03, wave 2)
- [x] 10-07-PLAN.md — widened WebKit/Firefox browser matrix (COMPAT-04, wave 3)
- [x] 10-08-PLAN.md — BROWSER_SUPPORT.md true floor + degradation matrix (COMPAT-05, wave 4)
...
**Plans**: 3/3 plans executed (waves 1-3)

Plans:
**Wave 1**

- [x] 11-01-PLAN.md — TRACER: size-baseline.mjs `--enforce` + soak-staged CI size gate (GATE-01, GATE-03, wave 1)
...
- [x] 11-03-PLAN.md — build-cost-cards.mjs + docs/cost-cards.md + drift-gate (DOCS-04, wave 3)
...
| 11. Gate Enforcement & Cost Publication | 3/3 | Complete    | 2026-08-31 |
